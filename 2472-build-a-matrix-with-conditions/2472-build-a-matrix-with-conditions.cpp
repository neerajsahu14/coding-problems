...
private:
    bool dfs(int src, unordered_map<int, vector<int>>& graph,
        ...
        for (int neighbor : graph[src]) {
            if (!dfs(neighbor, graph, visited, cur_path,
                     res)) // if any child returns false
                return false;
        }
    ...
    vector<int> topo_sort(unordered_map<int, vector<int>>& graph, int k) {
        ...
        reverse(res.begin(), res.end()); // we will have res as reversed so we
                                         // need to reverse it one more time
        return res;
    }
};
